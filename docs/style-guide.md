...
### 2.3 `__all__` для контроля публичного API
...
## 4. Константы и магические числа

### 4.1 Правило: ноль магических чисел в коде
...
### 4.3 Enum для категорий значений
...
## 5. Python-сахар: когда использовать, а когда нет

### 5.1 Списковые включения (list comprehensions)
...
### 5.4 Walrus-оператор `:=` (Python 3.8+)
